...
-- 25. Find locations where the wind speed is higher than the average wind speed.
SELECT location_id, wind_speed
FROM Weather_Data
WHERE wind_speed > (SELECT AVG(wind_speed) FROM Weather_Data);

-- 26. Calculate the moving average of temperature for each location over the last 7 days.
...
-- 27. Identify locations that experienced a temperature drop of more than 5 degrees Celsius within an hour.
...
-- 28. Find the location with the maximum temperature variation within a day.
...
-- 29. Display Real-Time Weather Conditions for Different Locations:
...
-- 30. Calculate Trends and Patterns in Weather Data Over Time:
